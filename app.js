...
mongoose.connect('mongodb+srv://admin:Entropia1@cluster0.193v2.mongodb.net/sudokuDatabase?retryWrites=true&w=majority');
...
app.use('/register', routerRegister, express.static(
  path.join(__dirname, 'views'), {
    index: 'register.html'
  })
);

app.use('/', routerLogin, express.static(
  path.join(__dirname, 'views'), {
    index: 'login.html'
  })
);
...
routerLogin.post('/:url', async (req, res)=>{
  ...
  app.use(`/${idUser}`, routerSudoku, express.static(
    path.join(__dirname, 'views'), {
      index: 'sudoku.html'
    })
  );
